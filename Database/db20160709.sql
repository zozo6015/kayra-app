...
CREATE TABLE `dogs` (
  ...
  `DogBreed` varchar(45) DEFAULT NULL,
...
INSERT INTO `dogs` VALUES (139,'Staffie','Bruno','Bruce','Roy','0987899876','bruce@gmail.com','www.facebook.com/bruce','Dog is beautiful',NULL,'2016-07-04 20:01:17'),(140,'Jack Russel','Abby','Lynn','Demeyer','0986564783','lynn@gmail.com','www.facebook.com/lynn','test',NULL,'2016-07-09 14:15:22'),(141,'poodle','Brujo','adsa','asd','23123','sadas@asd.com','sfsdf','sdfs',NULL,'2016-07-04 20:11:34'),(142,'Worshond','Jessie','Jenine','Martin','3243242','jenine@gmail.com','www.facebook.com/jenine','Cute dog',NULL,'2016-07-06 12:11:54');
...
INSERT INTO `users` VALUES (1,'Bruce1','Roy1','badroy','casino','bruceroy@gmail.com1','3234231','1',0,NULL,NULL,1),(66,'Peter','Kayra','peterk','123456','peter@gmail.com','234234234234','',1,'2016-07-06 13:51:24',NULL,1);
...
BEGIN
	...
	SELECT A.AppointmentID, A.DogID, D.DogBreed, D.DogName, D.OwnerFirstName, D.OwnerSurname, D.TelNo, D.Email, D.Facebook, D.Notes AS DogNotes, D.DogImg, GROUP_CONCAT(S.ServiceID) AS ServicesID, GROUP_CONCAT(S.ServiceDesc) AS Services, A.StartTime, A.EndDate, A.AppNotes, 'Day' AS Period
    FROM appointments A
    ...
    UNION ALL
    SELECT A.AppointmentID, A.DogID, D.DogBreed, D.DogName, D.OwnerFirstName, D.OwnerSurname, D.TelNo, D.Email, D.Facebook, D.Notes AS DogNotes, D.DogImg, GROUP_CONCAT(S.ServiceID) AS ServicesID, GROUP_CONCAT(S.ServiceDesc) AS Services, A.StartTime, A.EndDate, A.AppNotes, 'Week' AS Period
    FROM appointments A
    INNER JOIN dogs D ON D.DogID = A.DogID
    INNER JOIN dogservice DS ON DS.AppointmentID = A.AppointmentID
    INNER JOIN services S ON S.ServiceID = DS.ServiceID
    ...
    GROUP BY A.AppointmentID
    UNION ALL
    SELECT A.AppointmentID, A.DogID, D.DogBreed, D.DogName, D.OwnerFirstName, D.OwnerSurname, D.TelNo, D.Email, D.Facebook, D.Notes AS DogNotes, D.DogImg, GROUP_CONCAT(S.ServiceID) AS ServicesID, GROUP_CONCAT(S.ServiceDesc) AS Services, A.StartTime, A.EndDate, A.AppNotes, 'Month' AS Period
    FROM appointments A
    INNER JOIN dogs D ON D.DogID = A.DogID
    INNER JOIN dogservice DS ON DS.AppointmentID = A.AppointmentID
    INNER JOIN services S ON S.ServiceID = DS.ServiceID
...
BEGIN
	...
	SELECT DogID, DogBreed, DogName, OwnerFirstName, OwnerSurname, TelNo, Email, Facebook, Notes
    FROM dogs
    WHERE (_email = '' OR Email = _email)
    AND (_dogname = '' OR DogName = _dogname)
    AND (_dogbreed = '' OR DogBreed = _dogbreed)
    AND (_ownername = '' OR OwnerFirstName = _ownername)
    AND (_ownersurname = '' OR OwnerSurname = _ownersurname)
    AND (_ownertel = '' OR TelNo = _ownertel)
...
CREATE DEFINER=`kayra`@`%` PROCEDURE `SP_GetAllDogs`(
	_DogName VARCHAR(45),
    _DogBread VARCHAR(45),
    _FirstName VARCHAR(45),
    _Surname VARCHAR(45),
    _Email VARCHAR(45)
)
BEGIN
	SELECT DogID, DogBreed, DogName, OwnerFirstName, OwnerSurname, TelNo, Email, Facebook, Notes, DogImg, AuditDate
    FROM dogs
    WHERE (_DogName = '' OR DogName LIKE CONCAT('%', _DogName, '%'))
    AND (_DogBread = '' OR DogBreed LIKE CONCAT('%', _DogBread, '%'))
    AND (_FirstName = '' OR OwnerFirstName LIKE CONCAT('%', _FirstName, '%'))
    AND (_Surname = '' OR OwnerSurname LIKE CONCAT('%', _Surname, '%'))
    AND (_Email = '' OR Email LIKE CONCAT('%', _Email, '%'))
    ORDER BY DogName;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
BEGIN
	...
	SELECT A.AppointmentID, A.DogID, D.DogBreed, D.DogName, D.OwnerFirstName, D.OwnerSurname, D.TelNo, D.Email, D.Facebook, D.Notes AS DogNotes, D.DogImg, GROUP_CONCAT(S.ServiceID) AS ServicesID, GROUP_CONCAT(S.ServiceDesc) AS Services, A.StartTime, A.EndDate, A.AppNotes
    FROM appointments A
    INNER JOIN dogs D ON D.DogID = A.DogID
    INNER JOIN dogservice DS ON DS.AppointmentID = A.AppointmentID
    INNER JOIN services S ON S.ServiceID = DS.ServiceID
...
BEGIN
	...
	SELECT D.DogID, D.DogBreed, D.DogName, D.OwnerFirstName, D.OwnerSurname, D.TelNo, D.Email, D.Facebook, D.Notes, D.DogImg, A.AppNotes, StartTime AS AppStartDate, EndDate AS AppEndDate
    FROM dogs D
...
-- Dump completed on 2016-07-09 14:40:33
